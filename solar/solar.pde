...
DataSet smallData = new DataSet(500);
DataSet largeData = new DataSet(500);
...
void setup() {
  ...
  if (testMode==0) {
        ...
        while(true) {
          ...
          if ((text!=null) && (text.length()>2)) {
            ...
            if ("c".equals(action)) {
              ...
              println(s+","+l+","+ll);
              try {
                int small =Integer.parseInt(s);
                int large = Integer.parseInt(l);
                int lightlevel = Integer.parseInt(ll);
                float small_voltage=(5.0/1024.0)*small;
                float large_voltage=(5.0/1024.0)*large;
                //println(capacitance);
                smallData.addSample(small_voltage);
                largeData.addSample(large_voltage);
              } 
    ...
    serialThread = new Thread(new Runnable() {
      ...
      public void run() {
        while(true) {
          smallData.addSample(sin(number++/(4*PI)));
          delay(50);
        }
      }
    }
...
void draw() {
  ...
  synchronized(smallData) { 
    ...
    float mi = 0;
    float ma = max(smallData.getMax(),largeData.getMax());
    float span = ma-mi;
    ...
    println("min:"+mi+" ("+lower+") - max:"+ma+" ("+upper+") = "+span+" (steps:"+steps+" / text:"+textSteps+") @ "+magnitude+","+factor);
    ...
    float sampleSteps = 1.0 / smallData.getNumberOfSamples();
    for (int i=1; i < smallData.getNumberOfSamples(); i++) {
      float sample = norm(smallData.getSample(i), lower, upper);
      float previous = norm(smallData.getSample(i-1), lower, upper);
    ...
    sampleSteps = 1.0 / largeData.getNumberOfSamples();
    for (int i=1; i < largeData.getNumberOfSamples(); i++) {
      float sample = norm(largeData.getSample(i), lower, upper);
      float previous = norm(largeData.getSample(i-1), lower, upper);
