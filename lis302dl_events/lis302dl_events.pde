...
//events
String[] eventNames = { 
  "Click X", "Double Click X", "Click Y", "Double Click Y", "Click Z", "Double Click Z"};
int eventLineColor = 0x40bb30;
int eventFillColor = 0x88c039;
float eventAlpha = 1.0;
int numEvents = eventNames.length;
EventSet events = new EventSet(dataSetSize,eventNames);
        ...
        while(true) {
          ...
          if ((text!=null) && (text.length()>2)) {
            ...
            if (tokenizer.countTokens()>=6){
              ...
              String i1v = tokenizer.nextToken();
              String i2v  = tokenizer.nextToken();
              String cv  = tokenizer.nextToken();
              //println(c+","+d+" -> "+v);
              try {
                int x =Integer.parseInt(xv);
                int y = Integer.parseInt(yv);
                int z = Integer.parseInt(zv);
                int i1= Integer.parseInt(i1v);
                int i2= Integer.parseInt(i2v);
                int cl = Integer.parseInt(cv);
                boolean[] eventValues = new boolean[numEvents];
                eventValues[0]= (cl & 1) != 0;
                eventValues[1]= (cl & 2) != 0;
                eventValues[2]= (cl & 4) != 0;
                eventValues[3]= (cl & 8) != 0;
                eventValues[4]= (cl & 16) != 0;
                eventValues[5]= (cl & 32) != 0;
                events.addEvents(eventValues);
                dataSet.addSample(z*0.018);
...
void draw() {
  ...
  synchronized(dataSet) { 
    ...
    float mi = -4.1;//dataSet.getMin();
    float ma = 4.1;//dataSet.getMax();
    float span = ma-mi;
    //drawing the text
    ...
    for (int i=1; i < dataSet.getNumberOfSamples(); i++) {
      ...
      //draw the events
      stroke(eventLineColor, eventAlpha);
      fill(eventFillColor, eventAlpha);
      rect(-0.5,-0.5,1.0,1.0);
      float eventSteps = 1/numEvents;
/*      for (int e=0; e< numEvents; e++) {
        float eventLeft = map(i,0,dataSet.getNumberOfSamples(),left,right);
        float eventRight= map(i+1,0,dataSet.getNumberOfSamples(),left,right);
        float eventBottom = map(e,0,numEvents,bottom,top);
        float eventTop = map(e+1,0,numEvents,bottom,top);
        rect(0,1,0,1);
      }*/
    }
